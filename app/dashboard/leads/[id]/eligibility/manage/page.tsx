...
import { getQuestionsByType } from "@/actions/question"
import { ManageEligibilityQuestions } from "@/components/manage-eligibility-questions"
...
export default async function ManageEligibilityQuestionsPage({ params }: Props) {
  ...
  const currentQuestionsResult = await getAssessmentQuestions(assessment.id, "ELIGIBILITY")
  const currentQuestions = currentQuestionsResult.success
    ? (Array.isArray(currentQuestionsResult.data) ? currentQuestionsResult.data : [])
    : []

  // Get template questions from global bank (for reference/quick add)
  const templateQuestionsResult = await getQuestionsByType("ELIGIBILITY")
  const templateQuestions = templateQuestionsResult.success ? templateQuestionsResult.data : []
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 md:space-y-6">
      ...
      {/* Instructions */}
      <div className="glass rounded-xl p-4 md:p-6">
        <h3 className="text-sm md:text-base font-semibold mb-2">Manage Eligibility Questions</h3>
        <p className="text-sm text-foreground/70 mb-2">
          Customize eligibility questions for this specific company. You can:
        </p>
